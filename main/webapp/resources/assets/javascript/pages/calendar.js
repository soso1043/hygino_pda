	...
	var imgDir = "assets/images/";			// directory for images ... e.g. var imgDir="/img/"
	...
	var	imgsrc = new Array("down.gif","arrow_sm_gr.png","arrow.gif","left1.png","right1.png","right2.gif");
	var	img	= new Array();
    ...
    */
	...
	if (dom){
		...
		document.write ("<div onclick='bShow=true' id='calendar' style='z-index:+999;position:absolute;visibility:hidden;'><table cellspacing='0' width="+((showWeekNumber==1)?250:150)+" style='font-family:Noto Sans KR;font-size:14px;border:solid 1px #D6DBDF;box-shadow:0px 0px 3px #ccc;border-radius:4px'><tr style='border-width:1;background-color:#fff'><td><table width='"+((showWeekNumber==1)?248:205)+"'><tr><td style='padding:5px 0px 0px 11px; font-size:14px;'><B><span id='caption'></span></B></font></td><td align=right><a href='javascript:hideCalendar()' style='padding:5px'></a></td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#ffffff><span id='content'></span></td></tr>");
		
	
		document.write ("</table></div><div id='selectYear' style='z-index:+999;position:absolute;visibility:hidden;padding-left:26px;'></div><div id='selectMonth' style='z-index:+999;position:absolute;visibility:hidden;padding-left:57px;'></div>");
	}
	...
	var	styleLightBorder="color:#fff;background-color:#5A8DEE;";/*선택날짜*/
	...
	function init()	{
		if (!ns4){
			...
			sHTML1="<span id='spanLeft' style='border-width:0;cursor:pointer;' onmouseover='swapImage(\"changeLeft\",\"left1.png\");window.status=\""+scrollLeftMessage+"\"' onclick='javascript:decMonth()' onmouseout='clearInterval(intervalID1);swapImage(\"changeLeft\",\"left1.png\");window.status=\"\"' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartDecMonth()\",500)' onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeLeft' SRC='"+imgDir+"left1.png' width=6 height=10 BORDER=0>&nbsp</span>&nbsp;";
			// link to year drop-down list
			sHTML1+="<span id='spanYear' style='padding-left:23px;padding-right:10px;border-width:0;cursor:pointer' onmouseover='swapImage(\"changeYear\",\"arrow_sm_gr.png\");window.status=\""+selectYearMessage+"\"' onmouseout='swapImage(\"changeYear\",\"arrow_sm_gr.png\");window.status=\"\"' onclick='popUpYear()'></span>&nbsp;";
			// link to month drop-down list 
			sHTML1+="<span id='spanMonth' style='padding-right:23px;border-width:0;cursor:pointer'	onmouseover='swapImage(\"changeMonth\",\"arrow_sm_gr.png\");window.status=\""+selectMonthMessage+"\"' onmouseout='swapImage(\"changeMonth\",\"arrow_sm_gr.png\");window.status=\"\"' onclick='popUpMonth()'></span>&nbsp;";
			// link to next month 
			sHTML1+="<span id='spanRight' style='border-width:0;cursor:pointer'	onmouseover='swapImage(\"changeRight\",\"right1.png\");window.status=\""+scrollRightMessage+"\"' onmouseout='clearInterval(intervalID1);swapImage(\"changeRight\",\"right1.png\");window.status=\"\"' onclick='incMonth()' onmousedown='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"StartIncMonth()\",500)'	onmouseup='clearTimeout(timeoutID1);clearInterval(intervalID1)'>&nbsp<IMG id='changeRight' SRC='"+imgDir+"right1.png'	width=6 height=10 BORDER=0>&nbsp</span>&nbsp";
...
//		var	sTmp;
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			document.getElementById("selectMonth").innerHTML = "<table width=50	style='font-size:13px; border:1px solid #85929E;' bgcolor='white' cellspacing=0 onmouseover='clearTimeout(timeoutID1)'	onmouseout='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"popDownMonth()\",100);event.cancelBubble=true'>" +	sHTML +	"</table>";
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			document.getElementById("selectYear").innerHTML	= "<table width=44 style='font-size:13px; border:1px solid #85929E;'bgcolor='white' onmouseover='clearTimeout(timeoutID2)' onmouseout='clearTimeout(timeoutID2);timeoutID2=setTimeout(\"popDownYear()\",100)' cellspacing=0>"	+ sHTML	+ "</table>";
   ...
   function WeekNbr(n) {
	...
	function constructCalendar() {
		...
		sHTML =	"<table	 border=0 style='font-family:Noto Sans KR;font-size:14px;'><tr>";
		...
		for	(var i=0; i<7; i++)	{
			if(dayName[i]=="일"){
				sHTML += "<td width='27' align='right'><B>"+ dayName[i]+"</font></B></td>";
			}else if(dayName[i]=="토"){
				sHTML += "<td width='27' align='right'><B>"+ dayName[i]+"</font></B></td>";
			}else{
		...
		for	( var datePointer=1; datePointer<=numDaysInMonth; datePointer++ ){
			...
			if ((datePointer==dateNow)&&(monthSelected==monthNow)&&(yearSelected==yearNow)){ 
				...
				sHTML += "<a "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected="+datePointer + ";closeCalendar();'>&nbsp;" + datePointer + "</font>&nbsp;</a>";
		...
		document.getElementById("spanMonth").innerHTML = "&nbsp;" +	monthName[monthSelected] + "&nbsp;<IMG id='changeMonth' SRC='"+imgDir+"arrow_sm_gr.png' WIDTH='10' HEIGHT='6' BORDER=0>";
		document.getElementById("spanYear").innerHTML =	"&nbsp;" + yearSelected	+ "&nbsp;<IMG id='changeYear' SRC='"+imgDir+"arrow_sm_gr.png' WIDTH='10' HEIGHT='6' BORDER=0>";
	}
